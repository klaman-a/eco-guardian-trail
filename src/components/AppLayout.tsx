...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const { selectedSite, setSelectedSite, isGlobalView, isAuditView, isSiteView, viewMode, setViewMode } = useSiteContext();

  const notifications = useMemo(() => {
    if (isAuditView) return []; // Auditors don't get action notifications
    ...
    return items;
  }, [isGlobalView, isAuditView, isSiteView, selectedSite]);

  const navItems = [
    { to: '/', label: 'Dashboard', icon: LayoutDashboard, show: true },
    { to: '/assessments', label: 'Assessments', icon: FileText, show: true },
    { to: '/assessment-summary', label: 'Assessment Summary', icon: ClipboardCheck, show: isSiteView },
    { to: '/admin', label: 'Admin', icon: Settings, show: isGlobalView },
  ];

  const visibleNav = navItems.filter(item => item.show);

  const selectorValue = viewMode === 'audit' ? '__audit__' : viewMode === 'global' ? '__global__' : (selectedSite ?? '__global__');
  ...
  const handleSelectorChange = (v: string) => {
    if (v === '__global__') {
      ...
      setSelectedSite(v as any);
    }
  };

  const viewIcon = isAuditView ? <ScanEye className="h-4 w-4 text-amber-500" /> :
    isGlobalView ? <Globe className="h-4 w-4 text-muted-foreground" /> :
    <Building2 className="h-4 w-4 text-primary" />;

  const avatarLabel = isAuditView ? 'AU' : isGlobalView ? 'MS' : 'EF';
    ...
    <div className="min-h-screen bg-background">
      ...
      {isAuditView && (
        <div className="bg-amber-500/10 border-b border-amber-500/20 px-4 py-1.5 flex items-center justify-center gap-2">
          <ScanEye className="h-3.5 w-3.5 text-amber-600" />
          <span className="text-xs font-medium text-amber-700">Audit View — Read-only access with audit trail visibility</span>
        </div>
      )}
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={selectorValue} onValueChange={handleSelectorChange}>
                <SelectTrigger className={cn("h-8 w-[200px] text-xs", isAuditView && "border-amber-500/30")}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-popover z-50">
                  ...
                  <SelectItem value="__audit__">
                    <span className="flex items-center gap-1.5"><ScanEye className="h-3 w-3" /> Audit View</span>
                  </SelectItem>
                  {SITES.map(s => (
            ...
            <div className={cn(
              ...
              isAuditView ? "bg-amber-500/10 text-amber-700" : "bg-primary/10 text-primary"
            )}>
              {avatarLabel}
            </div>
          </div>
