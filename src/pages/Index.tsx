...
const Dashboard = () => {
  ...
  const pendingAssessments = assessments.filter((a) => a.status === 'pending-review' || a.status === 'draft').length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard title="Total Assessments" value={assessments.length} subtitle={`${pendingAssessments} pending action`} icon={FileText} />
        <MetricCard title="Drug Substances" value={totalSubstances} subtitle={`${exemptCount} exempt`} icon={FlaskConical} />
        <MetricCard title="Compliant" value={compliantCount} subtitle="PEC/PNEC < 1.0" icon={CheckCircle2} variant="success" />
        <MetricCard title="Non-Compliant" value={nonCompliantCount} subtitle="CAPA required" icon={AlertTriangle} variant={nonCompliantCount > 0 ? 'danger' : 'success'} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-card rounded-lg border border-border shadow-sm p-5">
          <h3 className="font-semibold mb-3 flex items-center gap-2">
            <AlertTriangle className="h-4 w-4 text-danger" /> Outstanding Actions
          </h3>
          <div className="space-y-2.5">
            {allSubstances.filter(s => s.complianceStatus === 'non-compliant').map(s => (
              <div key={s.id} className="flex items-start gap-3 p-2.5 rounded-md bg-danger/5 border border-danger/10">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">PEC/PNEC = {s.pecPnec?.toFixed(2) ?? '—'} · Initiate CAPA</p>
                </div>
              </div>
            ))}
        ...
        <div className="bg-card rounded-lg border border-border shadow-sm p-5">
          ...
          <div className="space-y-2.5">
            {assessments.filter(a => a.status === 'pending-review' || a.status === 'draft').map(a => (
              <Link key={a.id} to={`/assessment/${a.id}`}
